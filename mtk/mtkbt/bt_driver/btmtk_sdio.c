...
static int btmtk_sdio_recv_rx_data(void)
{
	int ret = 0;
	u32 u32ReadCRValue = 0;
	int retry_count = 150;/*add for  bt driver registering issue*/
	...
	do {
		...
		/* msleep(1); */
		mdelay(10);/*mdelay(3) changed by  for bt driver registering issue*/
        if(retry_count < 10)
            pr_info("%s: retry_count = %d,wait , ret=%d\n", __func__, retry_count,ret);
...
static int btmtk_sdio_set_i2s(void)
{
	...
	u32 pinmux = 0;
	return 0;
#if 1  //mtksdio proble too long so remove btmtk_sdio_set_i2s 
